...
const CAT_EMOJI = (id) => {
  if (!id) return "✨";
  if (id === "promKing" || id.toLowerCase().includes("king") || id.toLowerCase().includes("male")) return "👑";
  if (id === "promQueen" || id.toLowerCase().includes("queen") || id.toLowerCase().includes("female")) return "💎";
  if (id.toLowerCase().includes("business")) return "💼";
  if (id.toLowerCase().includes("dance")) return "💃";
  if (id.toLowerCase().includes("music")) return "🎵";
  if (id.toLowerCase().includes("fashion")) return "👗";
...
function ResultsView({ nominationsData }) {
  ...
  const tally = useMemo(() => {
    ...
    nominationsData.forEach((n) => {
      ...
      map[n.category_id][n.nominee_name] = (map[n.category_id][n.nominee_name] || 0) + 1;
    });
    return map;
  }, [nominationsData]);
...
export default function PromAdminDashboard() {
  ...
  useEffect(() => {
    fetch("https://mobilix.com.ng/promawards/pullvotes.php")
      ...
      .then((res) => {
        if (res.success) {
          setNominationsData(res.nominations || []);
          setVotesOverTimeData(res.votesOverTime || []);
          setCategoryBreakdownData(res.categoryBreakdown || []);
          setTotalSubmissions(res.totalSubmissions || 0);
          setTopNominee(res.topNominee || "—");
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  const filtered = nominationsData.filter((n) => {
    const matchSearch =
      n.nominee_name.toLowerCase().includes(search.toLowerCase()) ||
  ...
  if (loading) {
    return (
      <div style={{ color: "#fff", background: "#0b0813", padding: "20px", minHeight: "100vh", fontFamily: "Poppins, sans-serif" }}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-logo {
          ...
          font-size: 18px; margin-bottom: 20px; flex-shrink: 0;
        }
        .nav-btn {
          width: 44px; height: 44px; border-radius: 12px;
          display: flex; align-items: center; justify-content: center;
          font-size: 18px; cursor: pointer; transition: all 0.18s;
          background: transparent; border: none; position: relative; color: #fff;
        }
        .nav-btn:hover { background: rgba(255,255,255,0.06); }
        .nav-btn.active { background: rgba(124,58,237,0.22); border: 1px solid rgba(124,58,237,0.35); }
        ...
        .bottom-nav-btn {
          ...
          font-family: 'Poppins', sans-serif; transition: all 0.15s; min-width: 44px;
        }
        ...
        .content-area { flex: 1; display: flex; flex-direction: row; overflow: hidden; min-width: 0; }
        ...
        .right-panel {
          width: 280px; flex-shrink: 0; padding: 28px 20px;
          border-left: 1px solid rgba(255,255,255,0.05);
          background: #120f1e; display: flex; flex-direction: column;
          gap: 22px; overflow-y: auto;
        }

        /* Top bar */
        .topbar { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
        .topbar h1 { font-size: 1.55rem; font-weight: 800; letter-spacing: -0.02em; color: #fff; line-height: 1.1; }
        .topbar p { font-size: 11px; color: rgba(255,255,255,0.3); font-weight: 400; margin-top: 3px; }
        .topbar-right { display: flex; align-items: center; gap: 10px; }
        .admin-chip {
          display: flex; align-items: center; gap: 8px;
          background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09);
          border-radius: 100px; padding: 5px 14px 5px 8px;
          font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.7); white-space: nowrap;
        }
        .avatar {
        ...
        .panel-toggle-btn {
          ...
          border-radius: 100px; padding: 5px 12px; font-size: 11px; font-weight: 600;
          color: #c4b5fd; cursor: pointer; font-family: 'Poppins', sans-serif; white-space: nowrap;
        }
        ...
        .tab-btn:hover { border-color: rgba(124,58,237,0.4); color: #c4b5fd; }
        .tab-btn.active { background: rgba(124,58,237,0.2); border-color: rgba(124,58,237,0.5); color: #c4b5fd; }

        /* Stats */
        .stats-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; }
        .stat-card {
          background: #16131f; border: 1px solid rgba(255,255,255,0.07);
        ...
        .stat-label { font-size: 10px; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(255,255,255,0.35); margin-bottom: 10px; }
        ...
        /* Panels */
        .panel { background: #16131f; border: 1px solid rgba(255,255,255,0.07); border-radius: 18px; overflow: hidden; }
        .panel-head {
          padding: 18px 22px 14px; border-bottom: 1px solid rgba(255,255,255,0.05);
        ...
        /* Chart row */
        .chart-row { display: grid; grid-template-columns: 1fr 260px; gap: 14px; }
        ...
        .search-box:focus { border-color: rgba(124,58,237,0.5); box-shadow: 0 0 0 3px rgba(124,58,237,0.1); }
        .filter-pill {
          padding: 6px 14px; border-radius: 100px; font-size: 11px; font-weight: 600;
          cursor: pointer; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09);
          color: rgba(255,255,255,0.45); transition: all 0.15s; font-family: 'Poppins', sans-serif;
        }
        ...
        th {
          text-align: left; font-size: 10px; font-weight: 600; letter-spacing: 0.09em;
          text-transform: uppercase; color: rgba(255,255,255,0.3); padding: 0 12px 12px; white-space: nowrap;
        }
        td {
          padding: 13px 12px; font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.7);
          border-bottom: 1px solid rgba(255,255,255,0.04); white-space: nowrap;
        }
        tbody tr:last-child td { border-bottom: none; }
        tbody tr { transition: background 0.15s; }
        tbody tr:hover td { background: rgba(255,255,255,0.03); }
        .badge { display: inline-block; padding: 3px 10px; border-radius: 100px; font-size: 10px; font-weight: 600; border: 1px solid transparent; }
        ...
        .feed-item { display: flex; align-items: flex-start; gap: 10px; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.05); }
        ...
        /* Mobile side panel */
        .side-panel-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 200; }
        .side-panel-drawer {
          position: absolute; right: 0; top: 0; bottom: 0; width: min(300px, 88vw);
          background: #120f1e; border-left: 1px solid rgba(255,255,255,0.07);
          overflow-y: auto; padding: 20px 18px 80px;
          display: flex; flex-direction: column; gap: 22px;
        }
        .drawer-close { display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px; }
        .drawer-close-btn {
          ...
          <main className="main">
            ...
            <div className="tab-bar">
              {[
                ...
                { key: "records",  label: "🗳 Vote Records" },
              ].map((t) => (
                <button
                  key={t.key}
              ...
              <>
                <div className="stats-row">
                  ...
                  <div className="stat-card stat-subs">
                    ...
                    <div className="stat-sub">Most votes overall</div>
                  </div>
                </div>
            ...
            {activeTab === "results" && (
              <div className="panel" style={{ overflow: "visible" }}>
                ...
                <div className="panel-body">
                  <ResultsView nominationsData={nominationsData} />
                </div>
              </div>
            )}
